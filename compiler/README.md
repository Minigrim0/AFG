...
8. [Stack Allocation](#stack-allocation)
9. [Address resolution](#address-resolution)
10. [Final assembly generation](#final-assembly-generation)
...
## Register allocation
Rule of thumb, between two basic blocks, a register is supposed non-live. This means that register allocation is only done basic-block wise and no value live through a block transition inside a register.

This step should iterate through the temporary variables in the basic blocks, identifying if the variable is a good candidate to be replaced by a register.

e.g. in the following example:
```pasmfg
load @temp_oplpar_0 #1
add @temp_oplpar_0 #1
store ['SBP - 1] @temp_oplpar_0
```

The variable `@temp_oplpar_0` can be replaced with register `'GPA` (Given that the rest of the basic block does not use the variable). This will avoid allocating this variable on the stack.
Since these variable are generated by the translation module (in the `pasm` module) in the context of an operation or a comparison, we know that these variable are not supposed
to outlive the current block.
